//  Womens Data;

// var womensData = [
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "WOMEN'S REEBOK PRINT LEGGINGS ",
//       price: 1549,
//       strikedoffprice: 1799,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "WOMEN'S REEBOK POLYTIGHT",
//       price: 1249,
//       strikedoffprice: 1749,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "WOMEN'S REEBOK SLEEVE TEE",
//       price: 699,
//       strikedoffprice: 1245,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "WOMEN'S REEBOK PP TIGHTS",
//       price: 749,
//       strikedoffprice: 1299,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "WOMEN'S REEBOK PP TIGHTS",
//       price: 749,
//       strikedoffprice: 1299,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "WOMEN'S REEBOK CLASSIC LEGGINGS",
//       price: 1399,
//       strikedoffprice: 2799,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "WOMEN'S REEBOK CLASSIC TIGHTS",
//       price: 1099,
//       strikedoffprice: 1249,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "UNISEX REEBOK SHOW SHOES",
//       price: 1249,
//       strikedoffprice: 1489,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "WOMEN'S CLASSIC REEBOK PANTS",
//       price: 1459,
//       strikedoffprice: 1235,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "WOMEN'S REEBOK RUNNING RUN ESSENTIALS SHORT SLEEVE GRAPHIC",
//       price: 849,
//       strikedoffprice: 2599,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "WOMEN'S REEBOK TRAINING WOR MESH TIGHTS",
//       price: 1550,
//       strikedoffprice: 2599,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "WOMEN'S REEBOK RUNNING OSR HIGH-IMPACT BRA",
//       price: 1399,
//       strikedoffprice: 6599,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "UNISEX REEBOK TRAINING ACTIVE CORE BACKPACK - MEDIUM",
//       price: 949,
//       strikedoffprice: 1899,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "WOMEN'S REEBOK PRINT LEGGINGS ",
//       price: 1549,
//       strikedoffprice: 1799,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "WOMEN'S REEBOK POLYTIGHT",
//       price: 1249,
//       strikedoffprice: 1749,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "WOMEN'S REEBOK SLEEVE TEE",
//       price: 699,
//       strikedoffprice: 1245,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "WOMEN'S REEBOK PP TIGHTS",
//       price: 749,
//       strikedoffprice: 1299,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "WOMEN'S REEBOK PP TIGHTS",
//       price: 749,
//       strikedoffprice: 1299,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "WOMEN'S REEBOK CLASSIC LEGGINGS",
//       price: 1399,
//       strikedoffprice: 2799,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "WOMEN'S REEBOK CLASSIC TIGHTS",
//       price: 1099,
//       strikedoffprice: 1249,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "UNISEX REEBOK SHOW SHOES",
//       price: 1249,
//       strikedoffprice: 1489,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "WOMEN'S CLASSIC REEBOK PANTS",
//       price: 1459,
//       strikedoffprice: 1235,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "WOMEN'S REEBOK RUNNING RUN ESSENTIALS SHORT SLEEVE GRAPHIC",
//       price: 849,
//       strikedoffprice: 2599,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "WOMEN'S REEBOK TRAINING WOR MESH TIGHTS",
//       price: 1550,
//       strikedoffprice: 2599,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "WOMEN'S REEBOK RUNNING OSR HIGH-IMPACT BRA",
//       price: 1399,
//       strikedoffprice: 6599,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "UNISEX REEBOK TRAINING ACTIVE CORE BACKPACK - MEDIUM",
//       price: 949,
//       strikedoffprice: 1899,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "WOMEN'S REEBOK PRINT LEGGINGS ",
//       price: 1549,
//       strikedoffprice: 1799,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "WOMEN'S REEBOK POLYTIGHT",
//       price: 1249,
//       strikedoffprice: 1749,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "WOMEN'S REEBOK SLEEVE TEE",
//       price: 699,
//       strikedoffprice: 1245,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "WOMEN'S REEBOK PP TIGHTS",
//       price: 749,
//       strikedoffprice: 1299,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "WOMEN'S REEBOK PP TIGHTS",
//       price: 749,
//       strikedoffprice: 1299,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "WOMEN'S REEBOK CLASSIC LEGGINGS",
//       price: 1399,
//       strikedoffprice: 2799,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "WOMEN'S REEBOK CLASSIC TIGHTS",
//       price: 1099,
//       strikedoffprice: 1249,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "UNISEX REEBOK SHOW SHOES",
//       price: 1249,
//       strikedoffprice: 1489,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "WOMEN'S CLASSIC REEBOK PANTS",
//       price: 1459,
//       strikedoffprice: 1235,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "WOMEN'S REEBOK RUNNING RUN ESSENTIALS SHORT SLEEVE GRAPHIC",
//       price: 849,
//       strikedoffprice: 2599,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "WOMEN'S REEBOK TRAINING WOR MESH TIGHTS",
//       price: 1550,
//       strikedoffprice: 2599,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "WOMEN'S REEBOK RUNNING OSR HIGH-IMPACT BRA",
//       price: 1399,
//       strikedoffprice: 6599,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "UNISEX REEBOK TRAINING ACTIVE CORE BACKPACK - MEDIUM",
//       price: 949,
//       strikedoffprice: 1899,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "WOMEN'S REEBOK PRINT LEGGINGS ",
//       price: 1549,
//       strikedoffprice: 1799,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "WOMEN'S REEBOK POLYTIGHT",
//       price: 1249,
//       strikedoffprice: 1749,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "WOMEN'S REEBOK SLEEVE TEE",
//       price: 699,
//       strikedoffprice: 1245,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "WOMEN'S REEBOK PP TIGHTS",
//       price: 749,
//       strikedoffprice: 1299,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "WOMEN'S REEBOK PP TIGHTS",
//       price: 749,
//       strikedoffprice: 1299,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "WOMEN'S REEBOK CLASSIC LEGGINGS",
//       price: 1399,
//       strikedoffprice: 2799,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "WOMEN'S REEBOK CLASSIC TIGHTS",
//       price: 1099,
//       strikedoffprice: 1249,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "UNISEX REEBOK SHOW SHOES",
//       price: 1249,
//       strikedoffprice: 1489,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "WOMEN'S CLASSIC REEBOK PANTS",
//       price: 1459,
//       strikedoffprice: 1235,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "WOMEN'S REEBOK RUNNING RUN ESSENTIALS SHORT SLEEVE GRAPHIC",
//       price: 849,
//       strikedoffprice: 2599,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "WOMEN'S REEBOK TRAINING WOR MESH TIGHTS",
//       price: 1550,
//       strikedoffprice: 2599,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "WOMEN'S REEBOK RUNNING OSR HIGH-IMPACT BRA",
//       price: 1399,
//       strikedoffprice: 6599,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "UNISEX REEBOK TRAINING ACTIVE CORE BACKPACK - MEDIUM",
//       price: 949,
//       strikedoffprice: 1899,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "WOMEN'S REEBOK PRINT LEGGINGS ",
//       price: 1549,
//       strikedoffprice: 1799,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "WOMEN'S REEBOK POLYTIGHT",
//       price: 1249,
//       strikedoffprice: 1749,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "WOMEN'S REEBOK SLEEVE TEE",
//       price: 699,
//       strikedoffprice: 1245,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "WOMEN'S REEBOK PP TIGHTS",
//       price: 749,
//       strikedoffprice: 1299,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "WOMEN'S REEBOK PP TIGHTS",
//       price: 749,
//       strikedoffprice: 1299,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "WOMEN'S REEBOK CLASSIC LEGGINGS",
//       price: 1399,
//       strikedoffprice: 2799,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "WOMEN'S REEBOK CLASSIC TIGHTS",
//       price: 1099,
//       strikedoffprice: 1249,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "UNISEX REEBOK SHOW SHOES",
//       price: 1249,
//       strikedoffprice: 1489,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "WOMEN'S CLASSIC REEBOK PANTS",
//       price: 1459,
//       strikedoffprice: 1235,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "WOMEN'S REEBOK RUNNING RUN ESSENTIALS SHORT SLEEVE GRAPHIC",
//       price: 849,
//       strikedoffprice: 2599,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "WOMEN'S REEBOK TRAINING WOR MESH TIGHTS",
//       price: 1550,
//       strikedoffprice: 2599,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "WOMEN'S REEBOK RUNNING OSR HIGH-IMPACT BRA",
//       price: 1399,
//       strikedoffprice: 6599,
//     },
//     {
//       image_url:
//         "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRqg80Bbl9t-D_xN83yBkaKoo6mash5q4ZlTQ&usqp=CAU",
//       name: "UNISEX REEBOK TRAINING ACTIVE CORE BACKPACK - MEDIUM",
//       price: 949,
//       strikedoffprice: 1899,
//     },
//   ];

  // localStorage.setItem("womens",JSON.stringify(womensData));

  let dataWomens= JSON.parse(localStorage.getItem("womens"))||[];
dataWomens.forEach((elem)=>{
    let mainDiv = document.querySelector("#main");
    let div = document.createElement("div");
    div.style.boxShadow=" rgba(0, 0, 0, 0.35) 0px 5px 15px"
    div.style.padding="15px"
    let img = document.createElement("img");
    img.style.width="100%"
    img.src=elem.image_url;
    let h4 = document.createElement("h4");
    h4.innerText=elem.name;
    let price = document.createElement("h5");
    price.innerText="$"+elem.price;
    let button = document.createElement("button");
    button.style.border="none";
    button.style.padding="5px";
    button.style.backgroundColor="red";
    button.style.color="white";
    button.innerText="Add to cart";

    button.addEventListener("click", function(){
      addCart(elem,index);
    });
    div.append(img,h4,price,button);
    mainDiv.append(div);
})

let cartArr=JSON.parse(localStorage.getItem("cart"))||[];
function addCart(elem,index){
  cartArr.push(elem);
  localStorage.setItem("cart",JSON.stringify(cartArr));
}