...
let outputArray = inputArray.map((elem) => {
  let obj = {};
  let cat = categoriesDrectory[elem.Category];
  let area = areasDirectory[elem.Area];
  obj.productId = elem.idMeal;
  obj.productTitle = elem.strMeal;
  obj.Category = cat;
  obj.Area = area;
  obj.Ingredients = [];
  for(let i=1; i<=10; i++)
  {
    elem[`strIngredient${i}`] && elem[`strMeasure${i}`] && obj.Ingredients.push({ingredient: elem[`strIngredient${i}`], measure:elem[`strMeasure${i}`]});
  }
  return obj;
})
